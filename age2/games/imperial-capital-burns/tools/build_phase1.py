...
FONT_LICENSE_PAYLOAD_NAME = "SourceHanSansSC-LICENSE.txt"
FONT_LICENSE_PAYLOAD_PATH = (
    f"root/assets/data/gui/font/{FONT_LICENSE_PAYLOAD_NAME}"
)
...
SOURCE_IMAGE_LOCK_SCHEMA = "age2-imperial-source-image-lock/v2"
...
def read_font_license(path: Path) -> bytes:
    """Return a non-empty UTF-8 font license or fail the build."""
    ...
    try:
        ...
        raise ValueError(f"font license must be UTF-8 text: {path}") from exc
    if not text.strip():
        raise ValueError(f"font license is empty: {path}")
    if "\x00" in text:
        raise ValueError(f"font license is not a plain text file: {path}")
    return data


def copy_font_license(source: Path, payload_font: Path) -> Path:
    """Copy the caller-supplied font license beside the packaged fonts."""

    data = read_font_license(source)
    payload_font.mkdir(parents=True, exist_ok=True)
    target = payload_font / FONT_LICENSE_PAYLOAD_NAME
    shutil.copyfile(source, target)
    if target.read_bytes() != data:
        raise RuntimeError(f"font license copy verification failed: {target}")
    return target


def require_font_license_for_payload(payload: Path) -> None:
    """Reject a payload containing project fonts without their license text."""

    font_root = payload / "root" / "assets" / "data" / "gui" / "font"
    packaged_fonts = [name for name in FONT_HASHES if (font_root / name).is_file()]
    if not packaged_fonts:
        return
    license_path = font_root / FONT_LICENSE_PAYLOAD_NAME
    try:
        read_font_license(license_path)
    except (FileNotFoundError, ValueError) as exc:
        raise RuntimeError(
            "font binaries are present but their non-empty license text is missing "
            f"or invalid: {license_path}"
        ) from exc
...
def read_source_image_lock(
    path: Path,
    *,
    expected_manifests: dict[str, Path],
) -> tuple[dict[str, dict[str, dict[str, str]]], str]:
    raw = path.read_bytes()
    lock_sha256 = hashlib.sha256(raw).hexdigest().upper()
    document = json.loads(raw.decode("utf-8-sig"))
    if document.get("schema") != SOURCE_IMAGE_LOCK_SCHEMA:
        raise ValueError(f"unsupported source-image lock schema: {path}")
    if document.get("original_pack_sha256") != PACK_SHA256:
        raise ValueError(f"source-image lock targets a different pack: {path}")
    namespaces = document.get("namespaces")
    if not isinstance(namespaces, dict) or set(namespaces) != set(expected_manifests):
        raise ValueError("source-image lock namespace set differs from the build contract")
    result: dict[str, dict[str, dict[str, str]]] = {}
    for namespace, manifest_path in expected_manifests.items():
        record = namespaces.get(namespace)
        if not isinstance(record, dict) or record.get("manifest") != manifest_path.name:
            raise ValueError(f"source-image lock manifest mismatch: {namespace}")
        manifest_sha256 = str(record.get("manifest_sha256") or "").upper()
        if manifest_sha256 != sha256(manifest_path):
            raise ValueError(f"source-image lock manifest hash mismatch: {namespace}")
        entries = record.get("entries")
        if not isinstance(entries, list) or not entries:
            raise ValueError(f"source-image lock has no entries: {namespace}")
        normalized: dict[str, dict[str, str]] = {}
        source_keys: set[str] = set()
        target_keys: set[str] = set()
        for entry in entries:
            if not isinstance(entry, dict):
                raise ValueError(f"invalid source-image lock entry: {namespace}")
            source = entry.get("source_en")
            target = entry.get("target_ja")
            ...
            source = safe_relative(source).as_posix()
            target = safe_relative(target).as_posix()
            source_key = source.casefold()
            target_key = target.casefold()
            if source_key in source_keys or target_key in target_keys:
                raise ValueError(f"duplicate source/target in source-image lock: {namespace}")
            source_keys.add(source_key)
            target_keys.add(target_key)
            digest = digest.upper()
            if len(digest) != 64 or any(char not in "0123456789ABCDEF" for char in digest):
                raise ValueError(f"invalid source-image SHA-256: {namespace}/{source}")
            normalized[source] = {"target_ja": target, "source_sha256": digest}
        result[namespace] = normalized
    return result, lock_sha256


def validate_copy_image_rows(
    ...
    target_keys: set[str] = set()
    for row in rows:
        source = safe_relative(row["source_en"]).as_posix()
        target = safe_relative(row["target_ja"]).as_posix()
        source_key = source.casefold()
        target_key = target.casefold()
        if source_key in source_keys or target_key in target_keys:
            raise ValueError("duplicate source/target image in copy manifest")
        source_keys.add(source_key)
        target_keys.add(target_key)
        declared[source] = target
    locked = {
        source: record["target_ja"] for source, record in source_records.items()
    }
    if declared != locked:
        ...
        changed = sorted(
            source
            for source in set(declared) & set(locked)
            if declared[source] != locked[source]
        )
        raise ValueError(
            "source-image lock mapping mismatch: "
            f"missing={missing} extra={extra} changed_targets={changed}"
        )
...
def copy_image_rows(
    ...
    validate_copy_image_rows(rows, source_records)
    for row in rows:
        source_relative = safe_relative(row["source_en"]).as_posix()
        record = source_records[source_relative]
        source = safe_join(source_root, source_relative)
        expected = record["source_sha256"]
        if not source.is_file():
            raise FileNotFoundError(source)
        if sha256(source) != expected:
            raise ValueError(f"source UI image hash mismatch: {source}")
        target = copy_image_slot(
            source_root, target_root, row["source_en"], row["target_ja"]
        )
        if sha256(target) != expected:
            raise RuntimeError(f"source UI image copy verification failed: {target}")
...
def copy_tda_boot_notice_rows(
    ...
    for row in rows:
        source = safe_join(source_root, row["source_tda"])
        target = safe_join(target_root, row["target_ja"])
        if not source.is_file():
            raise FileNotFoundError(source)
...
def render_character_name_rows(
    ...
    for row in rows:
        relative = safe_relative(row["target_ja"])
        render_character_name_card(
            safe_join(source_root, row["target_ja"]),
            row["zh_cn"],
            font_path,
            safe_join(target_root, row["target_ja"]),
        )
...
def render_location_date_rows(
    ...
    for row in rows:
        relative = safe_relative(row["source_relative"])
        source = safe_join(source_root, row["source_relative"])
        if not source.is_file():
            raise FileNotFoundError(source)
        base = safe_join(target_root, row["source_relative"])
...
def write_location_date_layout_audit(
    ...
    with output.open("w", encoding="utf-8-sig", newline="") as stream:
        ...
        for row in rows:
            relative = safe_relative(row["source_relative"])
            source_boxes = detect_text_line_boxes(
                safe_join(source_root, row["source_relative"])
            )
            base = safe_join(output_root, row["source_relative"])
...
def create_location_date_preview(
    ...
    for index, row in enumerate(rows):
        relative = safe_relative(row["source_relative"])
        image = Image.open(safe_join(card_root, row["source_relative"])).convert("RGB")
...
def write_installers(output: Path) -> None:
    """Write a hash-gated installer and a manifest-aware uninstaller."""
    install = r'''param(
    [string]$GameDir = ""
)
...
字体授权：payload/root/assets/data/gui/font/SourceHanSansSC-LICENSE.txt。
"""
...
4. 把 root 直接复制到下面的 data 目录，选择“合并目录并覆盖同名文件”：
...
注意：复制的是 payload 里面的 root，不是把 payload 整个套进 data。不要修改 Steam 游戏目录里的 obb/pack.bin。
"""
...
def write_manifest(
    output: Path,
    *,
    source_image_lock_name: str,
    source_image_lock_sha256: str,
) -> None:
    if safe_relative(source_image_lock_name).name != source_image_lock_name:
        raise ValueError("source-image lock manifest name must be a plain file name")
    if (
        len(source_image_lock_sha256) != 64
        or source_image_lock_sha256 != source_image_lock_sha256.upper()
        or any(char not in "0123456789ABCDEF" for char in source_image_lock_sha256)
    ):
        raise ValueError("source-image lock manifest SHA-256 is invalid")
    payload = output / "payload"
    require_font_license_for_payload(payload)
    files = []
    ...
    manifest = {
        ...
        "source_image_lock": {
            "file": source_image_lock_name,
            "sha256": source_image_lock_sha256,
        },
        "files": files,
    }
...
def validate_character_name_cards(
    ...
    expected = {safe_relative(row["target_ja"]) for row in rows}
    actual = {
...
def validate_location_date_cards(
    ...
    for row in rows:
        relative = safe_relative(row["source_relative"])
        base = safe_join(card_root, row["source_relative"])
        en = base.with_name(f"{base.stem}_en{base.suffix}")
...
def validate_tda_boot_notices(
    ...
    for row in rows:
        path = safe_join(texture_root, row["target_ja"])
...
def validate(
    ...
    read_font_license(payload / "gui" / "font" / FONT_LICENSE_PAYLOAD_NAME)
...
def create_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        required=True,
        type=Path,
        help="non-empty UTF-8 license text distributed with the Source Han Sans fonts",
    )
...
def main() -> int:
    ...
    # Perform all static lock/manifest checks before creating a staging tree.
    # Inputs are checked again while copying and the lock digest is rechecked
    # before the final payload manifest is committed.
    read_font_license(args.font_license)
    game = args.repo / "age2" / "games" / "imperial-capital-burns"
    translations = game / "translations"
    image_copy = game / "images" / "copy"
    ui_manifest = image_copy / "ui.ja-zh-Hans.tsv"
    data_spec_manifest = image_copy / "schema.tsv"
    image_rows = read_tsv(ui_manifest)
    data_spec_image_rows = read_tsv(data_spec_manifest)
    source_image_lock_path = image_copy / "source-image-lock.v2.json"
    source_image_records, source_image_lock_sha256 = read_source_image_lock(
        source_image_lock_path,
        expected_manifests={
            "data/gui/textures": ui_manifest,
            "data_spec/gui/textures": data_spec_manifest,
        },
    )
    validate_copy_image_rows(
        image_rows, source_image_records["data/gui/textures"]
    )
    validate_copy_image_rows(
        data_spec_image_rows,
        source_image_records["data_spec/gui/textures"],
    )
    ...
    copy_image_rows(
        ...
        textures,
        image_rows,
        source_image_records["data/gui/textures"],
    )
    boot_notice_rows = read_tsv(image_copy / "boot-notice.tsv")
    ...
    copy_font_license(args.font_license, payload_font_root)
    ...
    character_name_rows = read_tsv(image_copy / "character-name-cards.ja-zh-Hans.tsv")
    ...
    patcher = args.repo / "age2" / "tools" / "uistring" / "patch_uistring.py"
    run_checked(
        [
            sys.executable,
            str(patcher),
            ...
            "--changes",
            str(translations / "ui-strings.ja-zh-Hans.tsv"),
            "--output",
            str(patched_dec),
        ]
    )
    ...
    telop_rows = read_tsv(image_copy / "telop.ja-zh-Hans.tsv")
    ...
    shutil.rmtree(work)
    if sha256(source_image_lock_path) != source_image_lock_sha256:
        raise RuntimeError("source-image lock changed during build")
    write_manifest(
        args.output,
        source_image_lock_name=source_image_lock_path.name,
