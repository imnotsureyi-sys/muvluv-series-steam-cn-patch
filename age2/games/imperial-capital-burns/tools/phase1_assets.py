...
def safe_relative(value: str) -> Path:
    """Return one canonical slash-separated path that cannot escape a root."""

    return _safe_relative(value)


def safe_join(root: Path, value: str) -> Path:
    """Resolve a manifest path and prove that symlinks cannot escape the root."""

    resolved_root = root.resolve(strict=False)
    candidate = (resolved_root / _safe_relative(value)).resolve(strict=False)
    try:
        candidate.relative_to(resolved_root)
    except ValueError as exc:
        raise ValueError(f"unsafe relative path: {value}") from exc
    return candidate
...
def copy_image_slot(
    ...
    target = safe_join(payload_root, target_relative)
