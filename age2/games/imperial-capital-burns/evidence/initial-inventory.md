...
- appmanifest：Steam 库内 `steamapps/appmanifest_2630300.acf`
- 安装目录：由该 appmanifest 的 `installdir` 与当前 Steam 库根目录解析；不绑定制作者盘符
- 资源包：`obb/pack.bin`
...
- 外层为 FPD v2，使用 [`../../../tools/fpd/`](../../../tools/fpd/) 与对应 `Scrambler.cs` 只读解析/筛选提取。
- 正文表为 EGPACK，后续正文阶段才使用 [`../../../tools/egpack/`](../../../tools/egpack/)；本阶段不修改。
...
- 本作未发现需要走 RIO/CRsa 的目标；旧 rUGP 经验仅用于排除格式，不进入 AGE2 工具链。

## 四类清单

| 分类 | 路径/格式 | 证据 | 当时采用/计划的处理 |
|---|---|---|---|
| 非正文可翻译 | `root/assets/data/epk/uistring.epk` | JP 槽含菜单、设置、系统提示 | FSNr EPK + `age2/tools/uistring/patch_uistring.py` |
...
| 非正文可翻译 | `root/assets/data/gui/font/*` | 简中缺字需覆盖 | 当时复制 TDA font payload；现行构建还必须通过 [`localization/tools/font_coverage.py`](../../../../localization/tools/font_coverage.py) 的字形检查，并把已审核字体许可证纳入 manifest |
