...
This checklist governs new releases. The five AGE2 beta packages currently linked from the root README predate it and do not consistently satisfy every manifest, version-gate, rollback, checksum and font-notice requirement; see the [player guide](player-guide.md).
...
- Update [`release-index.json`](release-index.json) with the immutable asset name, byte size, SHA-256, install/overlay roots and known historical caveats.
