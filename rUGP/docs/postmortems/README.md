...
| [CRsa full-field audit](crsa-native-text-20260903.md) and [native-field increment](crsa-native-increment-20260904.md) | Which PF/PM text fields were omitted beyond the first 40 display gaps, and how are the reviewed fields rebound? | Sequential native parsing of all 466 original CRsa records and 22 current overrides, source-hash checks for 57,547 reviewed rows, reviewed manifests, strict writeback/readback tests and PF/PM runtime samples | PM native CRsa uses fixed-extent volume staging after a later-volume identity RUO reproduced error 831; sampling does not claim every story branch |
