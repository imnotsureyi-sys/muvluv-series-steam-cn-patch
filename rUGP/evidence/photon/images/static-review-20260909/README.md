...
[第二轮 PF 开场采集](runtime-capture-round-02.json) 记录 578 个事件、78 个普通
绑定成功提交并回读；目标兵装图片身份仍未观察到。用户补充的日文图片模式截图
同样显示英文 HUD，现仅登记实际素材对应关系待核验，撤回已确认替换失效的口头
判断。不将它计为通过、已确认 hook 故障或补齐故障。PF 正常 DLL 已恢复；
按用户要求结束本轮实机配合，整合后继续反查。
