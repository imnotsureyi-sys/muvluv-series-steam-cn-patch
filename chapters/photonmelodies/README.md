...
- 当前公开版本：《时空的欠片》全文对照表、术语表与术语基线（补丁尚未发布）
- 发布页：无
- native 行数：40,541
- 补丁状态：翻译/写回流程阶段
...
《A Shimmering Shard of Spacetime／時空の欠片（时空的欠片）》已完成两遍制汉化与全文审核，当前先公开可定位的 JP-CN 正文源表、本篇术语表和全量术语基线；这不代表已经完成游戏内写回、实机测试或补丁发布。
...
- [`patch-sources/photonmelodies_shard_of_spacetime_terminology_jp_cn.csv`](../../patch-sources/photonmelodies_shard_of_spacetime_terminology_jp_cn.csv)：417 项已确认或须结合语境使用的本篇专名、固定表达和人物称谓。
- [`patch-sources/photonmelodies_shard_of_spacetime_terminology_baseline_jp_cn.csv`](../../patch-sources/photonmelodies_shard_of_spacetime_terminology_baseline_jp_cn.csv)：2,321 项全量术语基线，包括 357 项确认、60 项语境型项目和 1,904 项候选。

公开正文表只保留稳定顺序、`stable_id`、RIO、scene、日文说话人、JP 原文和当前 CN 文本；内部批次、审核结论和问题单不公开。术语基线单独公开候选状态、计数、来源和首次定位证据，便于后续复核；其中 `candidate` 仅表示待关注词，不代表固定译法，也不会自动进入正式术语表。发布前另按已冻结术语对 31 行残留旧写法做了机械一致性统一，没有重新裁译剧情内容。
